...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fix translation_mapping lineage fields that carry source-format labels"
    )
    parser.add_argument(
        "--dry-run", action="store_true",
        help="Print what would be fixed without writing to KG",
    )
    ...
    for item in to_fix:
        ...
        # NOTE: These direct G.add_edge calls violate O-1 (factory-method invariant).
        # They should be replaced with factory methods (e.g. kg.add_instantiated_in /
        # kg.add_attributed_to) in a future refactor once those methods exist.
